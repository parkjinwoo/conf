...
PROMPT='%{$fg[magenta]%}%n%{$fg[yellow]%}@%{$fg[green]%}%m%{$reset_color%}%{$fg_bold[black]%}:%{$reset_color%}%{$fg[blue]%}${PWD/#$HOME/~}%{$reset_color%}$(git_prompt_info)$(git_prompt_status)${ret_status}
\$ '
...
ZSH_THEME_GIT_PROMPT_MODIFIED="%{$reset_color%}%{$fg[blue]%}●%{$reset_color%}"
ZSH_THEME_GIT_PROMPT_DELETED="%{$reset_color%}%{$fg[red]%}✖%{$reset_color%}"
ZSH_THEME_GIT_PROMPT_UNTRACKED="%{$reset_color%}%{$fg[red]%}⋯%{$reset_color%}"
